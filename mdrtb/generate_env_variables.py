...
def generate_envrionment_variables():
    ...
    DEBUG = False if mode.upper() == "Y" else True
    REST_API_BASE_IP = (
        "172.17.15.130:8080" if mode.upper() == "Y" else "46.20.206.173:38080"
    )
    with open(f"{utils.get_project_root()}/.env", "w", encoding="utf-8") as env_file:
        ...
        # env_file.write("\n")
        # env_file.write(
        #     f"REST_API_BASE_URL=http://{REST_API_BASE_IP}/openmrs/ws/rest/v1/"
        # )
...
generate_envrionment_variables()
